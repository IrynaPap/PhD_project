...
global count_matrix
count_matrix = {"realisations_count": 1,
                "matrix_mineral": "An"}
...
def calculations_velocities(pressure, temperature, velocity, h, x, y):
    ...
    def calculate_current_elast_modules(realisation, cm):
        ...
        for mi in mineral_exemplar:
            # k = k + 1
            min_mi = mineral[mi]
            r = (mineral[mi]["F1"] * mineral[mi]["F1"]) - 1
...
#убрать из цикла матричный минерал / все кроме матрично
        ...
        mm0 = 0
        for mi3 in mineral:
            if mi3 != count_matrix["matrix_mineral"]:
                min_mi3 = mineral[mi3]
        ...
        calc_effective_linear(kz, khm, km, k1, kh, single_realisation)
    ...
    #что то тут не так
        ...
        calc_effective_kg(ef_lr, single_realisation)

    # calcEffectiveKG(0.6659, 408)

    def calc_effective_kg(ef_lr, single_realisation):
        velocities = {}
        solutions_vector = []
        ...
        # print('ef_k/ef_g',h,ro, ef_k, ef_g)
    ...
    def select_right_realisations(vector_of_realisations, ef_vp, ef_vs, ro):
        ...
        for i in range(0, vector_length):
            ...
            len_mineral_keys = len(mineral_keys)
            for l in range(0, len_mineral_keys):
                # fileAll.write(str(mineral_keys[l]) + ' ')
                fileAll.write(str(mineral[mineral_keys[l]]["C1"]) + ' ')
            fileAll.write('\n')
            fileMod.write('\n')
            ...
            if math.fabs(value_round) <= 0.03:
                ...
                for oxygen in oxygens:
                    oxygen_conc = oxygen_concentration(oxygen)
